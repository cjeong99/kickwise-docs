...
This folder contains the three static pages Movli needs for App Store
...
| `[CONTACT EMAIL]` | A real, monitored email address (the app currently uses `movli.app.feedback@gmail.com` in `AppLinks.swift` — use that if it's real, or pick a different one and update both places) |
...
Update these to point at it instead of the `Movli.app` placeholder:
1. `Movli/Utilities/AppLinks.swift` — `privacyPolicy` and `termsOfUse`
